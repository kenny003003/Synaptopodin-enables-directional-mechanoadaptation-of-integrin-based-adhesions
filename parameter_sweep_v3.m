...
C_VALUES = [1.3,2.45];           %  control 1.3KO 0.5
goal1=[31, 49, 54];
goal2=[16, 26, 35];
% --- MODIFIED: Sweep AR values here ---
AR_VALUES = [1.4];
% --------------------------------------

% Simulation Control
N_TRIALS = 500;            % Cells per condition
N_FIBERS = 50;             % Fibers per cell
...
for c_idx = 1:length(C_VALUES)
    ...
    for ar_idx = 1:length(AR_VALUES)
        ...
        % Note: AR=1 will be 0.0, AR=2 and AR=3 will be 0.7
        if AR > 1.5
            polarization = 0.7;
        else
            polarization = 0.0;
        ...
        % As AR increases, 'a' decreases to maintain total area
        radius_base = 1.0;
        a_fixed = radius_base / sqrt(AR);
        b_axis = a_fixed * AR;
        % -----------------------------------------
        
        parfor f_idx = 1:length(FORCE_RATIOS)
...
% Expanded styles to handle more AR values
colors = {'b', 'r', 'k', 'g', 'm'};       
markers = {'o', 's', '^', 'd', 'x'}; % Circle, Square, Triangle, Diamond
lines = {'--', '-', '-.', ':'};       % Varied line styles
...
for c_idx = 1:length(C_VALUES)
    for ar_idx = 1:length(AR_VALUES)
        ...
        % Use mod to cycle styles if we exceed array length
        col_use = colors{mod(c_idx-1, length(colors)) + 1};
        mark_use = markers{mod(ar_idx-1, length(markers)) + 1};
        line_use = lines{mod(ar_idx-1, length(lines)) + 1};
...
plot([1,3.5,15],goal1/100)
hold on
plot([1,3.5,15],goal2/100)
hold on
